...
ws.addEventListener('open', () =>{
    ...
    ws.addEventListener('message',({data}) =>{
        ...
        {
            case false:
                updatePopup({text:'invalid login parameters'})
            break;
            default:
...
sydDOM.main_form_page = () =>{
    ...
    return createElement(
        ...
        [
            createElement('h1',{style:'font-family: "Oswald", sans-serif;font-weight:400'},["Login your account"]),
            createElement('p',{style:'color:lightgrey;text-align:center'},["Enter the field below to continue"]),
            sydDOM.info_box().addAttr({style:styleComponent.info_box({method:'add',style:{marginTop:'20px'}})}).addAttr({class:filter('email')?'warn':''}),
...
sydDOM.info_box = (text = 'Email') =>{
    let type = text.toLowerCase() === 'user name' ? 'name' : text.toLowerCase();
